...
function initPage() {
	...
	var actionUrl = PageUtility.getContextPath() + "/TestPaper/getTestPaperBrowseData.action";
	testPaperBrowseData = PageUtility.ajaxData(actionUrl, null);
...
function showTestPaperHeader() {
	...
	$(".examName").text(testPaperBrowseData.testPaper.name);
	$(".totalScore").text("总分：" + testPaperBrowseData.testPaper.totalScore + "分");
	
	//获取题目数量并显示
	var testPaperOptions = testPaperBrowseData.testPaper.testPaperOptions;
	var testPaperOptionList = $.parseJSON(testPaperOptions);
	var questionCount = 0;
	for (var i=0; i<testPaperOptionList.length; i++) {
		questionCount = questionCount + Number(testPaperOptionList[i].number);
	}
	$(".questionCount").text("题量：" + questionCount + "道");
}
...
function getQuestionHtmlBySingleSelect(testPaperQuestion, index) {
	...
	if (testPaperBrowseData.testPaper.questionOptionsSortType == QuestionOptionsSortTypeEnum.Fixed) {
	...
	if (testPaperBrowseData.testPaper.questionOptionsSortType == QuestionOptionsSortTypeEnum.Random) {
...
function showTestPaperQuestionListByOption(testPaperOption) {
	...
	for (var i=0; i<testPaperBrowseData.questionList.length; i++) {
		if (testPaperBrowseData.questionList[i].questionType == testPaperOption.questionType) {
			questionList.push(testPaperBrowseData.questionList[i]);
		}
	}
