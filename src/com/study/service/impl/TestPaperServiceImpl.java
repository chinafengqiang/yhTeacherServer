...
public class TestPaperServiceImpl implements TestPaperService {
	...
	public TestPaper createTestPaper(String name, String description, String category,
		...
		testPaper.setCanQueryAnswer(canQueryAnswer);
	...
	private void trySelectQuestion(TestPaper testPaper, Integer series, Question question) throws Exception {
		...
		testPaperQuestion.setNote(question.getNote());
	...
	public void autoSelectQuestionList(Long testPaperId, Long questionCategoryId, QuestionTypeEnum questionTypeEnum, 
		...
		//判断是否全套
		Integer first = 1;
		Integer last = testPaper.getTotalSeries();
	...
	public void adjustQuestion(Long testPaperQuestionId, Double score, Integer sortFlag, Long curManagerId) throws Exception {
		...
		testPaperQuestion.setName(sortFlag.toString());
	...
	public void importTestPaper(File file, Long curManagerId) throws Exception {
		...
		testPaper.setTestPaperOptions(StringUtility.fixDataListJson(testPaper.getTestPaperOptions()));
		testPaper.setCreatedTime(DateUtility.getCurDate());
		testPaper.setCreator(curManagerId);
	 ...
	 * @throws Exception
	 */
	public TestPaperBrowseData getTestPaperBrowseData(Long testPaperId, Integer series) throws Exception {
		
		//判断是否存在试卷编号
		if (testPaperId == null) {
			throw new Exception("无试卷编号！!");
		}
		
		//判断是否存在试卷编号
		if (series == null) {
			throw new Exception("无试卷套数！!");
		}
		...
		List<TestPaperQuestion> testPaperQuestionList = this.modelFactoryFacade.getTestPaperQuestionFactory().findListByTestPaperId_Series(testPaperId, series);

		TestPaperBrowseData data = new TestPaperBrowseData();
